...
class TestTemplates(WebserviceTest):  # pragma: no cover
    ...
    def testGetTemplate(self):
        ...
        if P3K:
            ...
            "the Loyal German-Shepherd'] (editable, locked)>]"
    ...
    def testNonExistantTemplate(self):
        ...
        try:
            ...
            self.assertEqual(
                ex.message, "There is no template called 'Non_Existant' "
                "at this service")
    ...
    def testTemplateConstraintParsing(self):
        ...
        expected = '[<TemplateBinaryConstraint: Company.name = Woolies '
        '(editable, locked)>]'
        self.assertEqual(expected, repr(t.editable_constraints))
        self.assertEqual(
            '<TemplateBinaryConstraint: Company.departments.name = '
            'Farm Supplies (non-editable, locked)>', repr(t.get_constraint("B")))
        ...
        self.assertEqual(
            '<TemplateBinaryConstraint: Company.departments.name = '
            'Farm Supplies (editable, on)>', repr(con))
        con.switch_off()
        self.assertTrue(con.editable and con.optional and con.switched_off)
        self.assertEqual(
            '<TemplateBinaryConstraint: Company.departments.name = '
            'Farm Supplies (editable, off)>', repr(con))
        con.switch_on()
        self.assertTrue(con.editable and con.optional and con.switched_on)
