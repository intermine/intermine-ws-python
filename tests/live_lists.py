...
class LiveListTest(unittest.TestCase):
    ...
    GUYS_NAMES = 'Alex Karim "Gareth Keenan" Foo Bar "Keith Bishop" \
                     Vincent Baz'
    ...
    def testListTagAdding(self):
        s = self.SERVICE
        t = self.TYPE
        tempList = s.create_list(self.GUYS_NAMES,
                                 t,
                                 description="Id string",
                                 tags=['test', 'tag-adding'])
        self.assertEqual(set(['test', 'tag-adding']), tempList.tags)
        tempList.add_tags("a-tag", "b-tag")
    ...
    def testUnicode(self):
        s = self.SERVICE
        t = self.TYPE
        tempList = s.create_list(self.UNICODE_NAMES,
                                 t,
                                 description='unicode names',
                                 tags=['test'])
        self.assertEqual(len(self.UNICODE_NAMES), tempList.size)
    ...
    def testListTagRemoval(self):
        ...
        tempList = s.create_list(self.GUYS_NAMES,
                                 ...
                                 tags=tags)
        self.assertEqual(set(tags), tempList.tags)
        tempList.remove_tags("a-tag", "c-tag")
        self.assertEqual(set(["b-tag", 'test']), tempList.tags)
        tempList.remove_tags("b-tag", "d-tag")
        self.assertEqual(set(['test']), tempList.tags)
    ...
    def testListTagUpdating(self):
        ...
        self.assertEqual(
            ...
            set(
                map(str, s._list_manager.add_tags(tempList,
                                                  ["a-tag", "b-tag"]))))
        self.assertEqual(set(['test']), tempList.tags)
        tempList.update_tags()
        self.assertEqual(set(['test', "a-tag", "b-tag"]),
                         set(map(str, tempList.tags)))
    ...
    def test_context_manager(self):
        ...
        with self.SERVICE.list_manager() as m:
            ...
            a = m.create_list(self.LADIES_NAMES,
                              t,
                              description=desc("a"),
                              tags=tags)
            ...
            self.LOG.debug("ALL names before b: {0}".format(
                m.get_all_list_names()))
            b = m.create_list(self.GUYS_NAMES,
                              t,
                              description=desc("b"),
                              tags=tags)
            ...
            self.LOG.debug("ALL names before c: {0}".format(
                m.get_all_list_names()))
            c = m.create_list(self.EMPLOYEE_FILE,
                              t,
                              description=desc("c"),
                              tags=tags)
    ...
    def test_ladies_names(self):
        ...
        tempList = s.create_list(self.LADIES_NAMES,
                                 t,
                                 description="Id list",
                                 tags=["Foo", "Bar", "test"])
        self.assertEqual(tempList.unmatched_identifiers, set(["Zop", "Quux"]))
        self.assertEqual(tempList.size, 5)
        self.assertEqual(tempList.list_type, t)
        self.assertEqual(tempList.tags, set(["Foo", "Bar", "test"]))

        tempList = s.get_list(tempList.name)
        self.assertEqual(tempList.size, 5)
        self.assertEqual(tempList.list_type, t)
    ...
    def test_guys_names(self):
        ...
        tempList = s.create_list(self.GUYS_NAMES,
                                 ...
                                 description="Id string",
                                 tags=["Foo", "Bar", "test"])
        self.assertEqual(tempList.unmatched_identifiers,
                         set(["Foo", "Bar", "Baz"]))
        self.assertEqual(tempList.size, 5)
        self.assertEqual(tempList.list_type, "Employee")
        self.assertEqual(tempList.tags, set(["Foo", "Bar", "test"]))
    ...
    def test_from_file(self):
        ...
        tempList = s.create_list(self.EMPLOYEE_FILE,
                                 t,
                                 description="Id file",
                                 tags=["Foo", "Bar", "test"])
        self.assertEqual(tempList.unmatched_identifiers,
    ...
    def test_from_query(self):
        ...
        tempList = s.create_list(q, description="Id query", tags=['test'])
        self.assertEqual(tempList.unmatched_identifiers, set())
        self.assertEqual(tempList.size, 18)
        self.assertEqual(tempList.list_type, t)
    ...
    def test_renaming(self):
        ...
        tempList = s.create_list(q,
                                 description="test renaming",
                                 tags=["test", "query"])
        old_name = tempList.name

        tempList.name = "the list previously known as {0}".format(old_name)

        tempList2 = s.get_list(tempList.name)
        self.assertEqual(str(tempList), str(tempList2))
    ...
    def test_from_other_list(self):
        ...
        from_other = s.create_list(tempList)
        self.assertEqual(from_other.size, tempList.size)
    ...
    def test_to_query(self):
        ...
        got = [row[:3] + row[4:] for row in tempList.to_query().rows()]
    ...
    def test_iteration(self):
        ...
        # Test iteration:
        got = set([x.age for x in tempList])
        expected_ages = set([37, 41, 44, 53, 55])
        self.assertEqual(expected_ages, got)

        self.assertTrue(tempList[0].age in expected_ages)
        self.assertTrue(tempList[-1].age in expected_ages)
        self.assertTrue(tempList[2].age in expected_ages)
        self.assertRaises(IndexError, lambda: tempList[5])
        self.assertRaises(IndexError, lambda: tempList[-6])
        self.assertRaises(IndexError, lambda: tempList["foo"])
    ...
    def test_intersections(self):
        ...
        listA = s.create_list(self.GUYS_NAMES,
                              t,
                              description='test_intersections a',
                              tags=['test'])
        listB = s.create_list(self.EMPLOYEE_FILE,
                              t,
    ...
    def test_unions(self):
        ...
        listA = s.create_list(self.GUYS_NAMES,
                              ...
                              tags=['test', "tagA", "tagB"])
        listB = s.create_list(self.LADIES_NAMES,
                              t,
                              description='test_unions b',
                              tags=['test'])
    ...
    def test_appending_collection_of_lists(self):
        ...
        listA = s.create_list(self.GUYS_NAMES,
                              t,
                              description='appending_lists a',
                              tags=['test'])
        listB = s.create_list(self.EMPLOYEE_FILE,
                              t,
                              description='appending_lists b',
                              tags=['test'])
        listC = s.create_list(self.LADIES_NAMES,
                              t,
    ...
    def test_diffing(self):
        ...
        listA = s.create_list(self.GUYS_NAMES,
                              t,
                              description='test_diffing a',
                              tags=['test'])
        listB = s.create_list(self.EMPLOYEE_FILE,
                              t,
                              description='test_diffing b',
                              tags=['test'])
    ...
    def test_query_overloading(self):
        ...
        with_cc_l = s.create_list(with_cc_q,
                                  description=['test_query_overloading'])
