...
class LiveListTest(unittest.TestCase):
    ...
    def testListTagAdding(self):
        ...
        self.assertEqual(set(['test', 'tag-adding']), ls.tags)
        ls.add_tags("a-tag", "b-tag")
    ...
    def testUnicode(self):
        s = self.SERVICE
        t = self.TYPE
        ls = s.create_list(self.UNICODE_NAMES,
                           t,
                           description='unicode names',
                           tags=['test'])
        self.assertEqual(len(self.UNICODE_NAMES), ls.size)
    ...
    def testListTagRemoval(self):
        ...
        ls = s.create_list(self.GUYS_NAMES,
                           ...
                           tags=tags)
        self.assertEqual(set(tags), ls.tags)
        ls.remove_tags("a-tag", "c-tag")
        self.assertEqual(set(["b-tag", 'test']), ls.tags)
        ls.remove_tags("b-tag", "d-tag")
    ...
    def test_ladies_names(self):
        ...
        ls = s.create_list(self.LADIES_NAMES,
                           t,
                           description="Id list",
                           tags=["Foo", "Bar", "test"])
        self.assertEqual(ls.unmatched_identifiers, set(["Zop", "Quux"]))
        self.assertEqual(ls.size, 5)
        self.assertEqual(ls.list_type, t)
        self.assertEqual(ls.tags, set(["Foo", "Bar", "test"]))

        ls = s.get_list(ls.name)
        self.assertEqual(ls.size, 5)
        self.assertEqual(ls.list_type, t)
    ...
    def test_guys_names(self):
        ...
        ls = s.create_list(self.GUYS_NAMES,
                           ...
                           description="Id string",
                           tags=["Foo", "Bar", "test"])
        self.assertEqual(ls.unmatched_identifiers, set(["Foo", "Bar", "Baz"]))
        self.assertEqual(ls.size, 5)
        self.assertEqual(ls.list_type, "Employee")
        self.assertEqual(ls.tags, set(["Foo", "Bar", "test"]))
    ...
    def test_from_file(self):
        ...
        ls = s.create_list(self.EMPLOYEE_FILE,
                           ...
                           description="Id file",
                           tags=["Foo", "Bar", "test"])
        self.assertEqual(ls.unmatched_identifiers, set(["Not a good id"]))
        self.assertEqual(ls.size, 5)
        self.assertEqual(ls.list_type, "Employee")
        self.assertEqual(ls.tags, set(["Foo", "Bar", "test"]))
    ...
    def test_from_query(self):
        ...
        ls = s.create_list(q, description="Id query", tags=['test'])
        self.assertEqual(ls.unmatched_identifiers, set())
        self.assertEqual(ls.size, 18)
        self.assertEqual(ls.list_type, t)
    ...
    def test_renaming(self):
        ...
        ls.name = "the list previously known as {0}".format(old_name)

        ls2 = s.get_list(ls.name)
        self.assertEqual(str(ls), str(ls2))
