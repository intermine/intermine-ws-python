...
class QueryManagerTest(unittest.TestCase):
    ...
    def test_get_query(self):
        ...
        qm.post_query('<query name="yyy" model="genomic"\
                      ...
                      sortOrder="Gene.secondaryIdentifier ASC" >  <constraint path="Gene"\
    ...
    def test_delete_query(self):
        ...
        qm.post_query('<query name="xx" model="genomic" view="Gene.secondaryIdentifier \
                      Gene.symbol Gene.pathways.identifier " sortOrder="Gene.secondaryIdentifier\
    ...
    def test_post_query(self):
        ...
        self.assertEqual(qm.post_query('<query name="xx" model="genomic" view="Gene.secondaryIdentifier\ Gene.symbol Gene.pathways.identifier " sortOrder="Gene.secondaryIdentifier ASC" > \
                                       <constraint path="Gene" op="LOOKUP" value="bsk" extraValue="D.\ melanogaster" code="A" /> </query>'),'xx is posted')
        # deletes the query 'xx' we created to return to original state
        qm.delete_query('xx')
        # can't post if xml is wrong and returns a message
        self.assertEqual(qm.post_query('<query name="xxy" model="gnomic" view="Gene.secondaryIdentifier\ Gene.symbol Gene.pathways.identifier " sortOrder="Gene.secondaryIdentifier ASC" >  \
                                       <constraint path="Gene" op="LOOKUP" value="bsk" extraValue="D.\ melanogaster" code="A" /> </query>'), "Incorrect xml")
