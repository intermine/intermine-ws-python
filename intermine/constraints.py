...
class EmptyLogicError(ValueError):
    """
    An error representing the fact that an the logic string to be parsed was
    empty
    """
    pass
...
class LogicParser(object):
    ...
    def check_syntax(self, infix_tokens):
        ...
        This checks for:
         - correct operator positions (cannot put two codes next to each other
         without intervening operators)
         - correct grouping (all brackets are matched, and contain valid
         expressions)
            ...
            if token not in self.ops:
                ...
                if need_binary_op_or_closing_bracket:
                    raise LogicParseError(
                        "Logic grouping error after: '" + ' '.join(processed) +
        ...
        for token in infix_tokens:
            if token not in self.ops:
                ...
                if op == "(":
                    ...
                    while stack and self.get_priority(
                            stack[-1]) <= self.get_priority(op):
                        prev_op = stack.pop()
                        if prev_op != "(":
...
class TernaryConstraint(BinaryConstraint):
    ...
    def __init__(self, path, op, value, extra_value=None, code="A"):
        ...
        @param path: The path to constrain. Here is must be a class, or a
        ...
        @param extra_value: A further value for disambiguation. The meaning of
                            this value varies by class
                            and configuration. For example, if the class of
                            the object is Gene, then
...
class MultiConstraint(CodedConstraint):
    ...
    def __init__(self, path, op, values, code="A"):
        ...
        if not isinstance(values, (set, list)):
            raise TypeError("values must be a set or a list, not " +
                            str(type(values)))
        self.values = values
...
class RangeConstraint(MultiConstraint):
    ...
    """
    OPS = set([
        'OVERLAPS', 'DOES NOT OVERLAP', 'WITHIN', 'OUTSIDE', 'CONTAINS',
        'DOES NOT CONTAIN'
    ])
...
class TemplateConstraint(object):
    ...
    Constraints on templates can also be designated as "on", "off" or "locked",
    which refers to whether they are active or not. Inactive constraints are
    still configured, but behave as if absent for the purpose of results.
    In addition, template constraints can be editable or not. Only values for
    editable constraints can be provided when requesting results, and only
    constraints that can participate in logic expressions can be editable.
    """
    REQUIRED = "locked"
    OPTIONAL_ON = "on"
    OPTIONAL_OFF = "off"
    ...
    def separate_arg_sets(self, args):
        ...
        Splits a dictionary of arguments into two separate dictionaries,
        one with arguments for the main constraint, and one with arguments
        for the template portion of the behaviour
        """
        c_args = {}
        t_args = {}
...
class ConstraintFactory(object):
    ...
    CONSTRAINT_CLASSES = set([
        UnaryConstraint, BinaryConstraint, TernaryConstraint, MultiConstraint,
        SubClassConstraint, LoopConstraint, ListConstraint, RangeConstraint,
        IsaConstraint
    ])
