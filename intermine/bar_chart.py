...
def plot_go_vs_p(list_name):
    ...
    def autolabel(rects, ax):
        ...
        for rect in rects:
            ...
            y = rect.get_height()
            ax.annotate(gene_count[i], (x,y), xytext=(0,5), textcoords="offset points",
                        ha='center', va='bottom')
            i = i+1
...
def plot_go_vs_count(list_name):
    """
    A function to plot GO Term vs gene count with label of annotation on each bar
    ================================================
    example:
    ...
    def autolabel(rects, ax):
        ...
        for rect in rects:
            x = rect.get_x() + rect.get_width()/2.
            y = rect.get_height()
            ax.annotate(annotation_count[i], (x,y), xytext=(0,5), textcoords="offset points",
                        ha='center', va='bottom')
            i=i+1
...
def get_query(xml):
    ...
    r = requests.get(link)
    dict = json.loads(r.text)
    link = dict["instance"]["url"] + "/service/query/results?query="+ req.pathname2url(xml)
    r=requests.get(link)
    list=(r.text).split('\n')
...
def query_to_barchart_log(xml,resp):
    ...
    def autolabel(rects, ax):
        ...
        for rect in rects:
            x = rect.get_x() + rect.get_width()/2.
            y = rect.get_height()
            ax.annotate(y_val[i], (x,y), xytext=(0,5), textcoords="offset points",
                        ha='center', va='bottom')
            i = i+1
